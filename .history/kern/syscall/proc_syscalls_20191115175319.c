...
//COPY PROGRAM NAME FROM USER SPACE TO KERNEL ADDRESS SPACE (PROG PATH)
  char * prog_alloc_kern = kmalloc((strlen(progname) + 1) * sizeof(char));
  if (prog_alloc_kern == NULL) {
    panic("No program is allocated to kernel!");
  }
  int err = copyinstr((const_userptr_t) progname, (void *) prog_alloc_kern, (strlen(progname) + 1) * sizeof(char));
  if (!err) {
    panic("There was an error with the copy!");
  }
