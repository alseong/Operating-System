...
//COPY PROGRAM NAME FROM USER SPACE TO KERNEL ADDRESS SPACE (PROG PATH)
  ...
  int err = copyin((const_userptr_t) progname, prog_kern, prog_size);
  if (err) {
...
// count number of args and copy into kernel
  ...
  // char * program_name_kernel = kmalloc(prog_size);
  // KASSERT(program_name_kernel != NULL);
  // int err = copyin((const_userptr_t) prog_name, (void *) program_name_kernel, program_name_size);
  // KASSERT(err == 0);
