...
int sys_execv(const char * prog_name, char ** args)
{
  ...
  //NULL TERMINATED SO USE COPYINSTR
  int result = copyinstr((const_userptr_t) prog_name, (void *) prog_kern, PATH_MAX, NULL);
  if (result) {
    return result;
  }
  ...
  //NULL TERMINATED SO USE COPYINSTR
    int result = copyinstr((const_userptr_t) args[i], (void *) args_kernel[i], PATH_MAX, NULL);
    if (result) {
      return result;
    }
  }
  ...
  for (int i = args_count - 1; i >= 0; i--) {
    ...
    stackptr -= args_size;
    int result = copyoutstr((void *) args_kernel[i], (userptr_t) stackptr, args_size, NULL);
    if (result) {
      return result;
    }
    args_ptr[i] = stackptr;
  ...
  for (int i = args_count; i >= 0; i--) {
    ...
    int result = copyout((void *) &args_ptr[i], (userptr_t) stackptr, args_ptr_size);
    if (result) {
      return result;
    }
  }

  //NOW IT IS SAFE TO DELETE OLD ADDRESS AND FREE EVERYTHING
