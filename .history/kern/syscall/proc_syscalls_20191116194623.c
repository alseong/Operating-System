...
int sys_execv(const char * prog_name, char ** args)
{
  ...
  result = copyin((const_userptr_t) prog_name, (void *) prog_kern, prog_size);
  if (result) {
    return result;
  }
  ...
  for (int i = 0; i < args_count; i++) {
    ...
    result = copyin((const_userptr_t) args[i], (void *) args_kernel[i], arg_size);
    if (result) {
      return result;
    }
  }
  ...
  for (int i = args_count - 1; i >= 0; i--) {
    ...
    stackptr -= args_size;
    result = copyout((void *) args_kernel[i], (userptr_t) stackptr, args_size);
    if (result) {
      return result;
    }
    args_ptr[i] = stackptr;
