...
//COPY PROGRAM NAME FROM USER SPACE TO KERNEL ADDRESS SPACE (PROG PATH)
  size_t prog_kern = (strlen(progname) + 1) * sizeof(char);
  char * prog_kern = kmalloc(program_name_len);
  if (program_name_kernel == NULL) {
    panic("Program_kern is null!");
  }
  int err = copyin((const_userptr_t) progname, (void *) program_name_kernel, program_name_len);
  if (err) {
