...
//COPY PROGRAM NAME FROM USER SPACE TO KERNEL ADDRESS SPACE (PROG PATH)
  char * program_name_kernel = kmalloc((strlen(progname) + 1) * sizeof(char));
  KASSERT(program_name_kernel != NULL);
  int err = copyin((const_userptr_t) progname, (void *) program_name_kernel, (strlen(progname) + 1) * sizeof(char));
  KASSERT(err == 0);
