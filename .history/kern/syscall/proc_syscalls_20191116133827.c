...
//   args_kern[args_count] = NULL;
  ...
  // copy program args into kernel
  size_t args_ptr_size = (args_count + 1) * sizeof(char *);
  char ** args_kern = kmalloc(args_ptr_size);
  if (args_kern == NULL) {
    panic("args_kern is null!");
  }
  for (int i = 0; i <= args_count; i++) {
      size_t args_size = (strlen(args[i]) + 1) * sizeof(char);
      args_kern[i] = kmalloc(args_size);
      if (args_kern[i] == NULL) {
        panic("args_kernel index is NULL! \n");
      }
      int err = copyin((const_userptr_t) args[i], (void *) args_kern[i], args_size);
      if (err) {
        panic("There was an issue with copy!");
      }
  }
