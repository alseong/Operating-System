...
int sys_execv(const char * prog_name, char ** args)
{
  ...
  for (int i = args_count; i >= 0; i--) {
    ...
    stackptr -= arg_size;
    int err = copyout((void *) args_kernel[i], (userptr_t) stack_ptr, arg_length);
    KASSERT(err == 0);
  ...
  for (int i = args_count; i >= 0; i--) {
    ...
    int err = copyout((void *) &stack_args[i], (userptr_t) stack_ptr, str_pointer_size);
    KASSERT(err == 0);
  }
  // HARD PART: COPY ARGS TO USER STACK
  ...
  kfree(args_kernel);
	...
	enter_new_process(args_count /*argc*/, (userptr_t) temp_stack_ptr /*userspace addr of argv*/,
			  ROUNDUP(stack_ptr, 8), entrypoint);
