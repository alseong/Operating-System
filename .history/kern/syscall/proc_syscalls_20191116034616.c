...
//AFTER DELETE old dd space
  ...
  vaddr_t * args_ptrs = (vaddr_t *)kmalloc((argc + 1) * sizeof(vaddr_t));

 for (int i = args_count; i >= 0; i--) {
     size_t arg_size = ROUNDUP(strlen(args_kern[i]) + 1, 4);
     args_ptrs -= arg_size;
     int err = copyoutstr((void *) args_kernel[i], (userptr_t) stackptr, arg_size, &actual);
     KASSERT(err == 0);
  ...
  for (int i = args_count; i >= 0; i--) {
    ...
    int err = copyout((void *) &stack_args[i], (userptr_t) stackptr, str_pointer_size);
    KASSERT(err == 0);
  }
  ...
  kfree(args_kernel);
	...
	/* Warp to user mode. */
	enter_new_process(args_count, NULL /*userspace addr of argv*/,
			  stackptr, entrypoint);
