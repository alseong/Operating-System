...
vaddr_t * args_stack_ptrs = (vaddr_t *)kmalloc((args_count + 1) * sizeof(vaddr_t));

 for (int i = args_count - 1; i >= 0; i--) {
     size_t arg_size = ROUNDUP(strlen(args_kern[i]) + 1, 4);
     args_ptrs -= arg_size;
     int err = copyoutstr((void *) args_kern[i], (userptr_t) stackptr, arg_size, &actual);
     KASSERT(err == 0);
  ...
  for (int i = args_count; i >= 0; i--) {
    ...
    int err = copyout((void *) &args_ptrs[i], (userptr_t) stackptr, str_pointer_size);
    KASSERT(err == 0);
  }
