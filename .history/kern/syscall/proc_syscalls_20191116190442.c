...
int sys_execv(const char * prog_name, char ** args)
{
  ...
  size_t prog_size = (char *) kmalloc(strlen(char) * PATH_MAX); 
  char * prog_kern = kmalloc(strlen((char *) prog_name) + 1) * sizeof(char));
  if (prog_kern == NULL) {
    panic("Program_kern is null!");
  }
  int err = copyinstr((const_userptr_t) prog_name, (void *) prog_kern, prog_size, NULL);
  if (err) {
    return err;
  }
  ...
  for (size_t i = 0; i < args_count; i++) {
    ...
    int err = copyinstr((const_userptr_t) args[i], (void *) args_kernel[i], arg_size, NULL);
    if (err) {
      return err;
    }
  }
