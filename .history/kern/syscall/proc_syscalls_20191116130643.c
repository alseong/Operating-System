...
int sys_execv(const char * prog_name, char ** args)
{
  ...
  size_t program_name_size = (strlen(prog_name) + 1) * sizeof(char);
  char * program_name_kernel = kmalloc(program_name_size);
  KASSERT(program_name_kernel != NULL);
  int err = copyinstr((const_userptr_t) prog_name, (void *) program_name_kernel, program_name_size, &program_actual);
  KASSERT(err == 0);
  ...
  for (int i = 0; i <= num_args; i++) {
    if (i == num_args) {
      ...
      int err = copyinstr((const_userptr_t) args[i], (void *) args_kernel[i], arg_size, &args_actual);
      KASSERT(err == 0);
    }
  }
  ...
  for (int i = num_args; i >= 0; i--) {
    ...
    int err = copyoutstr((void *) args_kernel[i], (userptr_t) temp_stack_ptr, arg_length,&args_actual);
    KASSERT(err == 0);
  ...
  for (int i = num_args; i >= 0; i--) {
    ...
    int err = copyoutstr((void *) &stack_args[i], (userptr_t) temp_stack_ptr, str_pointer_size, &args_actual);
    KASSERT(err == 0);
  }
  // HARD PART: COPY ARGS TO USER STACK
