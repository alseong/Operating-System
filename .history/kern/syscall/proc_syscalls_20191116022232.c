...
//COPY PROGRAM NAME FROM USER SPACE TO KERNEL ADDRESS SPACE (PROG PATH)
  ...
  char * prog_kern = kmalloc((strlen(progname) + 1) * sizeof(char));
  if (prog_kern == NULL) {
...
// count number of args and copy into kernel
  ...
  size_t args_kern = (args_count + 1) * sizeof(char *);
  char ** args_kernel = (char **) kmalloc(args_kern);
  if (prog_kern == NULL) {
  ...
  for (int i = 0; i < args_count; i++) { 
      ...
      int err = copyinstr((const_userptr_t) args[i], args_kernel[i], arg_size, &arg_actual);
      KASSERT(err == 0);
  }
...
//by populating the new program's stack with all the args from the old program's stack  ) ( CREATE A SEPARATE FUNCTION FOR )
 ...
 vaddr_t *stack_args = (vaddr_t *) kmalloc((args_count + 1) * sizeof(vaddr_t));
size_t actual2;
for (int i = args_count; i >= 0; i--) {
    ...
    int err = copyoutstr((void *) args_kernel[i], (userptr_t) temp_stack_ptr, arg_length, &actual2);
    KASSERT(err == 0);
