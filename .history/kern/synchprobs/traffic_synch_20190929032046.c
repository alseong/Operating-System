...
intersection_before_entry(Direction origin, Direction destination) 
{
  ...
  for (int i = 0; i < array_num(in_intersection); i++) { //in_intersection may have changed after the car wakes up again
...
intersection_after_exit(Direction origin, Direction destination) 
{
  ...
  car->origin = origin;
  car->destination = destination;
  Vehicle *cur = array_get(in_intersection, i);
  for (unsigned int i = 0; i < array_num(in_intersection); i ++){
    if (in_intersection[i].origin == car.origin && in_intersection[i].destination == car.destination) {
