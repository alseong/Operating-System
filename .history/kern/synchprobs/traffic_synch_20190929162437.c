...
intersection_before_entry(Direction origin, Direction destination) 
{
  ...
  if (car->origin == north && North_accident != 0) {
    North_wait++;
    cv_wait(North_cv, intersection_lock);
  }
  else if (car->origin == south && South_accident != 0) {
    South_wait++;
    cv_wait(South_cv, intersection_lock);
  }
  else if (car->origin == east && East_accident != 0) {
    East_wait++;
    cv_wait(East_cv, intersection_lock);
  }
  else if (car->origin == west && west_accident != 0) {
    West_wait++;
